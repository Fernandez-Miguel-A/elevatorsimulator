#include "c22.h"
double c22::minimo(double val1, double val2){
if (val1 <= val2){
	return val1;
}else{
	return val2;
...
bool c22::disponibilidad(){
	return (e1.compare("Libre") == 0 || e2.compare("Libre") == 0);
}
double c22::optimo(std::string a1, std::string a2){
    if (a1.compare("Libre") == 0){
		return 1;
	}else{
		if (a2.compare("Libre") == 0){
			return 2;
		}else{	
			return 1;
		}	
...
bool c22::find(std::deque<double> list, double elem){
...
//clear the files
myfile.open ("c22_cola1_in.csv");
myfile << "";
myfile.close();
myfile.open ("c22_cola1_out.csv");
myfile << "";
myfile.close();
myfile.open ("c22_cola2_in.csv");
myfile << "";
myfile.close();
myfile.open ("c22_cola2_out.csv");
myfile << "";
myfile.close();

}
double c22::ta(double t) {
//This function returns a double.
printLog("\nTA controlador\n");
return minimo(tiempo1,tiempo2);
}
void c22::dint(double t) {
...
if((puerto_salida == 0) && (e1.compare("Ocupado") ==0) && (tiempo1 <= tiempo2) &&
 (piso1 == destino1) && (cola1.size() == 0)){
	...
	if ((puerto_salida == 0) && (e1.compare("Ocupado") ==0) && (tiempo1 <= tiempo2) &&
        ...
        myfile.open ("c22_cola1_out.csv",std::ios::app);
        myfile << e_char;
        myfile <<"\n";
        myfile.close();
...
printLog("IF4");
				...
				if((puerto_salida == 1) && (e2.compare("Ocupado") == 0) && (tiempo1 > tiempo2) &&
				 (piso2 ==destino2) && (cola2.size() !=0)){
                    ...
                    e_string<< t;
                    char const *e_char = e_string.str().c_str();
                    myfile.open ("c22_cola2_out.csv",std::ios::app);
                    myfile << e_char;
                    myfile <<"\n";
                    myfile.close();
...
void c22::dext(Event x, double t) {
...
double *aux;
aux = (double*) x.value;
if((e1.compare("Libre") == 0) && (x.port == 0) && (optimo(e1,e2) == 1)){
    counter_aux++;
	destino1 = *aux;
	e1 = "Ocupado";
	tiempo1 = 0;
	...
	if((e1.compare("Ocupado") == 0) && (x.port == 0) && (optimo(e1,e2) == 1)){
        tiempo1 = tiempo1 -e;
        if (!find(cola1,*aux)){
		    cola1.push_back(*aux);
            ...
            e_string<< t;
            char const *e_char = e_string.str().c_str();
            myfile.open ("c22_cola1_in.csv",std::ios::app);
            myfile << e_char;
            myfile <<"\n";
            myfile.close();
		...
		if((e2.compare("Libre") == 0) && (x.port == 0) && (optimo(e1,e2) == 2)){
            counter_aux++;
			tiempo1 = tiempo1 -e;
			destino2 = *aux ;
            ult_dest2 = *aux;			
			...
			if((e2.compare("Ocupado") == 0) && (x.port == 0) && (optimo(e1,e2) ==2)){
                tiempo1 = tiempo1-e;
				tiempo2 = tiempo2-e;
                printLog("ifffffff1");
                if (!find(cola2,*aux)){            	
                    ...
                    e_string<< t;
                    char const *e_char = e_string.str().c_str();
                    myfile.open ("c22_cola2_in.csv",std::ios::app);
                    myfile << e_char;
                    myfile <<"\n";
                    myfile.close();
